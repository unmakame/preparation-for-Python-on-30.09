...
class Parent:
    def __init__(self):
        self.value = 10  # Этот атрибут не создастся
...
child = Child()
print(child.value)  # ОШИБКА: нет атрибута 'value'
```
 ## миксин 
...
class A:
    def __new__(cls):
        ...
        return object()  # Возвращаем базовый object вместо A
    
    def __init__(self):
        print("Этот код НИКОГДА не выполнится!")

obj = A()
print(type(obj))  # <class 'object'> вместо <class 'A'>
```
...
b = np.arange(0, 5, 2)            # [0 2 4] 
c = np.linspace(0, 1, 3)          # [0.  0.5 1. ]
d = np.zeros(3)                   # [0. 0. 0.]
e = np.ones((2, 2))               # [[1. 1.] [1. 1.]]
f = np.full(3, 7)                 # [7 7 7]
g = np.random.rand(3)             # [0.1 0.5 0.9]
```
 ##
# 6 Билет
 ## сеттер
...
class Person:
    ...
    @age.setter
    def age(self, value):
        if value >= 0:  # Проверка перед присвоением
            self._age = value
```
 ## миксин 
...
# in-place
a = [1, 2, 3]
a.append(4)   # список изменяется "на месте"
print(a)      # [1, 2, 3, 4]
```
...
class Person:
    ...
    def get_name(self):     # геттер с другим именем
        return self._name

    def set_name(self, value):  # сеттер с другим именем
        self._name = value
```
...
Отнощения в UML это связи между классами или объектами, которые показывают, как элементы системы взаимодействуют друг с другом. Основные виды отношений:
 Наследование: "является" (is-a), стрелка с полым треугольником.
 Композиция: "имеет" (has-a), заполненный ромб, компонент не существует без композита.
 Агрегация: слабое "имеет", пустой ромб, компонент независим.
 Ассоциация: связь между классами, линия, возможно с кардинальностью (1, , 1..).
 Зависимость: временное использование, пунктирная стрелка.
 Реализация: класс выполняет интерфейс, пунктирная стрелка с полым треугольником.
...
class A:
    def __new__(cls):
        print("Создаем объект B вместо A")
        return B()  # возвращаем объект другого класса
...
b = a  # создаётся ещё одна ссылка
print(sys.getrefcount(a))

del a  # удаляем одну ссылку
print(sys.getrefcount(b))  # объект всё ещё жив, т.к. b ссылается на него
```
...
d.greet()
```
 ## написать класс person с приватными атрибутами name, age. написать для них сеттеры и геттеры (property)
 ```
class Person:
...
# 16 Билет
 ...
 ```
class Person:
    def __init__(name):  # забыли self
        name = name

p = Person("Alice")  # TypeError: __init__() takes 1 positional argument but 2 were given
```
